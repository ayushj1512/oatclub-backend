// routes/commerceFeed.js
import express from "express";
import Product from "../Products/Products.js";
import CommerceManager from "../CommerceManager/CommerceManager.js";
...
let cache = {
  ...
  etag: "",
  expiresAt: 0,
  meta: { count: 0, builtAt: null, selectedCodes: 0, matchedProducts: 0 },
};
...
async function rebuildFeed() {
  ...
  const selectedCodes = safeArr(config?.selectedProductCodes).map(normalizeCode);

  if (!config?.isActive || !selectedCodes.length) {
    const xml = buildXml("");
    const t = Date.now();
  ...
  for (const p of products) {
    ...
    if (isVariable && safeArr(p?.variants).length) {
      ...
      items.push(
        buildItemXml({
          id: String(p?.code || productId).trim(),
          itemGroupId: "",
          title,
          desc,
          link,
...
router.get("/test", async (req, res) => {
  try {
    ...
    return res.json({
      ok: true,
      selectedProductCodes: config?.selectedProductCodes || [],
      selectedCount: (config?.selectedProductCodes || []).length,
      isActive: !!config?.isActive,
    });
  } catch (e) {
    console.error("❌ Commerce feed test failed:", e?.message || e);
...
router.get("/xml", async (req, res) => {
  try {
    ...
    console.error("❌ Commerce XML feed failed:", e?.message || e);
    return res.status(500).send("Feed generation failed");
  }
});

/* manual refresh (for admin button) */
router.post("/commerce-feed/refresh", async (req, res) => {
  try {
    ...
    return res.json({
      ...
      selectedCodes: feed.meta.selectedCodes,
      matchedProducts: feed.meta.matchedProducts,
    });
  } catch (e) {
    console.error("❌ Commerce feed refresh failed:", e?.message || e);
    return res
      .status(500)
...
router.get("/commerce-feed/status", (req, res) => {
  return res.json({
    ok: true,
    count: cache.meta.count,
    builtAt: cache.meta.builtAt,
    selectedCodes: cache.meta.selectedCodes,
    matchedProducts: cache.meta.matchedProducts,
    cached: !!cache.xml,
...
/*
Mount:
app.use("/api/commerce-feed", commerceFeedRouter);

URLs:
GET  /api/commerce-feed/xml
GET  /api/commerce-feed/test
POST /api/commerce-feed/commerce-feed/refresh
GET  /api/commerce-feed/commerce-feed/status
*/